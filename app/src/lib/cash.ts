...
/** Archived accounts too — the Settings list shows them so they can be restored. */
export async function listAllBankAccounts(cinemaId: string): Promise<BankAccount[]> {
  const sb = getSupabase();
  if (!sb) return [];
  const { data, error } = await sb
    .from("bank_accounts")
    ...
    .order("created_at");
  if (error) {
    console.warn("[cash] listAllBankAccounts", error.message);
    return [];
  }
  return (data as BankAccountRow[] | null ?? []).map(mapBankAccount);
}

// ── Bank account management (owner-only; bank_accounts_owner_manage) ─────────
...
export interface BankAccountPatch {
  ...
  openingDate?:    DateISO | null;
  isPrimary?:      boolean;
}

/** Revise an account. Promoting one to primary demotes the rest (DB-side). */
export async function saveBankAccount(id: string, d: BankAccountPatch): Promise<void> {
  const sb = getSupabase();
  if (!sb) throw new Error("Supabase not configured");
  if (!d.name.trim()) throw new Error("The account needs a name.");
  const { error } = await sb.rpc("fn_bank_account_save", {
    p_id:              id,
    p_name:            d.name,
    p_bank_name:       d.bankName ?? null,
    p_last4:           d.accountLast4 ?? null,
    p_opening_balance: d.openingBalance ?? null,
    p_opening_date:    d.openingDate ?? null,
    p_is_primary:      d.isPrimary ?? null,
  });
  if (error) throw new Error(error.message);
}

/** What still points at this account — drives the "archived because…" copy. */
export interface BankAccountUsage { label: string; n: number }

export async function bankAccountUsage(id: string): Promise<BankAccountUsage[]> {
  const sb = getSupabase();
  if (!sb) return [];
  const { data, error } = await sb.rpc("fn_bank_account_usage", { p_id: id });
  if (error) { console.warn("[cash] bankAccountUsage", error.message); return []; }
  return (data as Array<{ label: string; n: number | string }> | null ?? [])
    .map((r) => ({ label: r.label, n: Number(r.n) }))
    .filter((r) => r.n > 0);
}

/**
 * Remove an account. An untouched one is really deleted; anything with history
 * behind it is archived instead (it leaves every picker, the history stays).
 * The DB decides — the return value says which happened.
 */
export async function deleteBankAccount(id: string): Promise<"deleted" | "archived"> {
  const sb = getSupabase();
  if (!sb) throw new Error("Supabase not configured");
  const { data, error } = await sb.rpc("fn_bank_account_delete", { p_id: id });
  if (error) throw new Error(error.message);
  return (data as "deleted" | "archived" | null) ?? "archived";
}

export async function restoreBankAccount(id: string): Promise<void> {
  const sb = getSupabase();
  if (!sb) throw new Error("Supabase not configured");
  const { error } = await sb.rpc("fn_bank_account_restore", { p_id: id });
  if (error) throw new Error(error.message);
}
