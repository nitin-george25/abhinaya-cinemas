...
/** POS counter (till) inside an operating unit. Migration 17. */
...
export interface DailyCashClosing {
  id:                      string;
  operatingUnitId:         string;
  /** POS counter this closing belongs to. Migration 17. */
  posCounterId:            string;
...
export interface ClosingDraft {
  operatingUnitId:     string;
  /** Required — closings are per POS counter from migration 17. */
  posCounterId:        string;
  businessDate:        DateISO;
  shift:               ClosingShift;
  cashierEmail?:       string | null;
 ...
 * shift) as the natural key (migration 17) — uses the unique constraint
 * to dedupe.
 */
...
export interface PettyDraft {
  operatingUnitId:    string;
  /** Required — petty expenses are per POS counter from migration 17. */
  posCounterId:       string;
  expenseDate:        DateISO;
  amount:             number;
  category?:          string | null;
