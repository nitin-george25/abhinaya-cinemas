-- =============================================================================
-- Movie de-duplication v2 (supersedes movie_dedup.sql).
-- Folds each duplicate movie into its canonical id, repoints entries + openings,
-- renames the survivor to its web-verified title, drops orphans from the movies
-- table AND the config.movies blob the app reads. Adrishyam/Drishyam and all
-- distinct films are left untouched. Idempotent/guarded; a no-op on a DB without
-- these ids. CI applies in timestamp order: this runs after the distributor
-- normalization (20260616120000) and before the distributors migration (20260616140000).
-- =============================================================================
begin;

create temp table dedup_map(bf text, canon text) on commit drop;
insert into dedup_map(bf, canon) values
  ('mh49b42c0d','cjnx0h3'),
  ('mhad7b9991','85m6eki'),
  ('mhbe14874d','vfj13iu'),
  ('mh4283fce3','87sa8pk'),
...
create temp table movie_rename(id text, title text) on commit drop;
insert into movie_rename(id, title) values
  ('cjnx0h3','Aadu 3'),
...
-- 3) Repoint openings (pre-tool opening balances) onto the canonical id.
...
delete from public.openings o using dedup_map m where o.movie_id = m.bf;  -- drop any leftover colliding openings
...
delete from public.movies where id in (select bf from dedup_map);

-- 6) Update the config.movies blob the app reads: drop orphans + rename survivors.
...
set data = jsonb_set(c.data, '{movies}', (
      ...
      where mv->>'id' not in (select bf from dedup_map)
    )),
    updated_by = 'movie-dedup-v2', updated_at = now()
where c.id = 1 and jsonb_typeof(c.data->'movies') = 'array';

-- 7) Guard: no entry/opening may still reference a removed id.
...
-- VERIFY: select id, name from public.movies order by name;  -- survivors renamed, no mh* dups