...
function totalsFor(bo: BoAgg[], fb: FbAgg[]): WeekTotals {
  ...
  return {
    ...
    totalTickets: bo.reduce((s, r) => s + r.tickets, 0),
    totalFbNet: fb.reduce((s, r) => s + r.net, 0),
...
function perScreen(bo: BoAgg[]): Array<{ screen: string; shows: number; tickets: number }> {
  const m: Record<string, { screen: string; shows: number; tickets: number }> = {};
  bo.forEach((r) => {
    if (!m[r.screenName]) m[r.screenName] = { screen: r.screenName, shows: 0, tickets: 0 };
...
function renderBody(opts: {
  ...
  curr: WeekTotals;
  prev: WeekTotals;
  byScreen: Array<{ screen: string; shows: number; tickets: number }>;
  ...
  const screenRowsHtml = byScreen.length ? byScreen.map((r) => `
    ...
    </tr>`).join("") : `<tr><td colspan="3" style="padding:14px;color:#999;text-align:center">No screens reported</td></tr>`;
...
function plainText(opts: { weekFrom: string; weekTo: string; cinemaName: string; curr: WeekTotals; prev: WeekTotals; byScreen: Array<{ screen: string; shows: number; tickets: number }>; topMov: { name: string; tickets: number } | null; byDay: Array<{ date: string; tickets: number; fbNet: number; hasBO: boolean; hasFB: boolean }>; }): string {
...
By screen:
${byScreen.map((r) => `  ${r.screen}: ${r.shows} shows, ${r.tickets} tickets`).join("\n")}
